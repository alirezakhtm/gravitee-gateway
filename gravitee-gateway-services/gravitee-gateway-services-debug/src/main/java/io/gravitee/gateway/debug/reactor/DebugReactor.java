...
public class DebugReactor extends DefaultReactor {
    ...
    public void onEvent(Event<ReactorEvent, Reactable> reactorEvent) {
        if (reactorEvent.type() == ReactorEvent.DEBUG) {
            ...
            DebugApi reactableDebugApi = toReactableDebugApi(reactableEvent);
            if (reactableDebugApi != null) {
                ...
                try {
                    eventOptional = eventRepository.findById(reactableDebugApi.getEventId());
                } catch (TechnicalException e) {
                    logger.error("An error occurred when searching event {}, removing the handler.", reactableDebugApi.getEventId(), e);
                    reactorHandlerRegistry.remove(reactableDebugApi);
                    return;
                }

                if (eventOptional.isEmpty()) {
                    logger.error("No event found for id {}, removing the handler.", reactableDebugApi.getEventId());
                    reactorHandlerRegistry.remove(reactableDebugApi);
                    return;
                }
    ...
    private DebugApi toReactableDebugApi(ReactableEvent event) {
        try {
